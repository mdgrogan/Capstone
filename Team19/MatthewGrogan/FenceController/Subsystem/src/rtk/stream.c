...
{    
	...
	fileptr = fopen("debug.txt", "a+");
	fprintf(fileptr, "(stream 5) entering openfile_\n");
	fclose(fileptr);
...
static file_t *openfile(const char *path, int mode, char *msg)
{
	...
	fileptr = fopen("debug.txt", "a+");
	fprintf(fileptr, "(stream 1) entering openfile\n");
	fclose(fileptr);
    
    if (!(mode&(STR_MODE_R|STR_MODE_W))) return NULL;
   
	fileptr = fopen("debug.txt", "a+");
	fprintf(fileptr, "(stream 2)\n");
	fclose(fileptr);
